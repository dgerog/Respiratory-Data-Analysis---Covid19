...
dataInd1 = np.where(study.isActive == True)
dataInd2 = np.where(study.isPatient == False)
dataInd  = np.concatenate((dataInd1[0], dataInd2[0]))
study.trimData(dataInd)
...
# Split age groups
classifier = svm.SVC(kernel='poly') # initialize classifier
